...
/**
 * 복수 터미널 도시(서울/대구/부산/대전/광주/수원/청주) 되묻기 흐름을 광범위하게 스트레스 테스트한다.
...
class ConversationParseServiceMultiTerminalTest {
    ...
    @Test
    void resolves_two_terminal_cities_like_suwon_and_cheongju() {
        // 수원, 청주처럼 터미널이 2개뿐인 도시도 동일한 되묻기 로직을 탄다.
        ConversationSession session = newSession("수원", "청주");

        ConversationParseResponse r1 = service.parse(new ConversationParseRequest("아무 말", "s1"), session);
        assertThat(r1.clarificationPrompt()).startsWith("수원 어느 터미널로");
        apply(session, r1);

        ConversationParseResponse r2 = service.parse(new ConversationParseRequest("서수원", "s1"), session);
        assertThat(r2.departure()).isEqualTo("서수원");
        assertThat(r2.arrival()).isEqualTo("청주");
        assertThat(r2.clarificationPrompt()).startsWith("청주 어느 터미널로");
        apply(session, r2);

        ConversationParseResponse r3 = service.parse(new ConversationParseRequest("북청주", "s1"), session);
        assertThat(r3.departure()).isEqualTo("서수원");
        assertThat(r3.arrival()).isEqualTo("북청주");
    }
    ...
    void wrong_city_standalone_answer_gives_a_specific_message_for_multiple_city_pairs() {
        ...
        ConversationSession session = newSession("부산", "광주");
        ConversationParseResponse r1 = service.parse(new ConversationParseRequest("아무 말", "s1"), session);
        assertThat(r1.clarificationPrompt()).startsWith("부산 어느 터미널로");
        apply(session, r1);

        // 광주 터미널("송정")을 부산 되묻기에 답해버린 경우
        ConversationParseResponse r2 = service.parse(new ConversationParseRequest("송정", "s1"), session);
        assertThat(r2.departure()).isEqualTo("부산"); // 안 바뀜
        assertThat(r2.arrival()).isEqualTo("광주");   // 안 바뀜
        assertThat(r2.clarificationPrompt()).doesNotContain("죄송해요");
        assertThat(r2.clarificationPrompt()).contains("광주").contains("부산");
    }

    @Test
    void a_third_unrelated_city_standalone_answer_does_not_silently_resolve_either_pending_direction() {
        // 출발/도착이 둘 다 애매한 상황(부산↔광주)에서, 둘 중 어디에도 안 속하는 제3의 도시(대구)로
        ...
        assertThat(r2.arrival()).isEqualTo("광주");
        assertThat(r2.clarificationPrompt()).doesNotContain("죄송해요");
    ...
    void once_the_currently_asked_direction_is_resolved_a_matching_city_answer_moves_to_the_other_direction() {
        // 부산↔광주가 둘 다 애매할 때, 먼저 부산을 정확히 답하고 나면(노포동), 그 다음 턴에는
        // 광주 터미널("송정")로 정확히 답했을 때 이번엔 진짜로 도착지에 반영되어야 한다 — 앞서 고친
        ...
        assertThat(r1.arrival()).isEqualTo("광주"); // 아직 그대로
        assertThat(r1.clarificationPrompt()).startsWith("광주 어느 터미널로");
        apply(session, r1);

        ConversationParseResponse r2 = service.parse(new ConversationParseRequest("송정", "s1"), session);
        assertThat(r2.departure()).isEqualTo("부산종합");
        assertThat(r2.arrival()).isEqualTo("광주송정");
    }
